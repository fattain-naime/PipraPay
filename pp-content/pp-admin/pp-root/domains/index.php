...
<div class="page-header d-print-none" aria-label="Page header">
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            <div class="col">
            <!-- Page pre-title -->
                <div class="page-pretitle">Domains</div>
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list align-items-center gap-3">
                    ...
                    <span data-bs-target="#modal-createItem" data-bs-toggle="modal" class="<?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'domains', 'whitelist', $global_user_response['response'][0]['role']) ? '' : 'd-none' ?>">
                ...
                <div class="form-group mt-1">
                    ...
                    <div class="form-control-wrap">
                        <select class="form-select" id="model-bulkActionID">
                            ...
                            <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'domains', 'edit', $global_user_response['response'][0]['role']) ? '<option value="activated">Activate Selected</option>' : '' ?>
                            <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'domains', 'edit', $global_user_response['response'][0]['role']) ? '<option value="inactive">Inactive Selected</option>' : '' ?>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="row mb-3 g-3">
              <div class="col-lg-12">
                ...
                <div class="input-group">
                    ...
                    <input type="text" class="form-control" name="domain-name" placeholder="Domain name">
                </div>
              </div>
            </div>
            ...
            <div class="form-selectgroup-boxes row g-3 mb-3">
              <div class="col-lg-6">
                <label class="form-selectgroup-item">
                  <input type="radio" name="domain-status" value="active" class="form-selectgroup-input" checked="">
                  <span class="form-selectgroup-label d-flex align-items-center p-3">
                    ...
                    <span class="form-selectgroup-label-content">
                      ...
                      <span class="d-block text-secondary">If <strong>Active</strong>, this domain can initiate payments.</span>
                    </span>
                  </span>
                </label>
              </div>
              <div class="col-lg-6">
                <label class="form-selectgroup-item">
                  <input type="radio" name="domain-status" value="inactive" class="form-selectgroup-input">
                  <span class="form-selectgroup-label d-flex align-items-center p-3">
                    <span class="me-3">
                      <span class="form-selectgroup-check"></span>
                    </span>
                    <span class="form-selectgroup-label-content">
                      ...
                      <span class="d-block text-secondary">If <strong>Inactive</strong>, this domain can't initiate payments.</span>
                    </span>
                  </span>
                </label>
              </div>
...
<script data-cfasync="false">
        ...
        if(actionID == ""){
            ...
            if(my_action_confirmation_btn !== ""){
                ...
                $.ajax({
                    ...
                    data: {action: "domain-bulk-action", csrf_token: csrf_token_default, actionID: actionID, selected_ids: JSON.stringify(selectedRows)},
                    dataType: 'json',
    ...
    function deleteItem(ItemID){
        ...
        if(my_action_confirmation_btn !== ""){
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo $site_url.$path_admin ?>/dashboard',
                data: {action: "domains-delete", csrf_token: csrf_token_default, ItemID: ItemID},
                dataType: 'json',
                success: function (response) {
                    closeAllBootstrapModals();
            ...
            show_action_confirmation_tab(btnClass, 'Delete Domain', 'Delete', 'btn-danger');
        }
    }
    ...
    function load_data_list(page = 1){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo $site_url.$path_admin ?>/dashboard',
            data: {action: "all-domain-list", csrf_token: csrf_token_default, search_input: search_input, show_limit: show_limit, page: page, filter_status: filter_status, filter_start: filter_start, filter_end: filter_end},
            dataType: 'json',
                    ...
                    res.response.forEach(item => {
                        ...
                        html += `
                            <tr data-id="${item.id}">
                                ...
                                <td ${redirectEdit}>${item.domain}</td>
    ...
    function openEditModel(itemID){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo $site_url.$path_admin ?>/dashboard',
            data: {action: "domains-info-byID", csrf_token: csrf_token_default, ItemID: itemID},
            dataType: 'json',
            success: function (response) {
                document.querySelector('.'+loaderSpinner).innerHTML = '';
                ...
                if (response.status === 'true') {
                    ...
                    // Set status radio
                    const statusInputs = modal.querySelectorAll('input[name="domain-status"]');
    ...
    $('.modal-createItem-btn').click(function () {
        ...
        var domain_status = statusInput ? statusInput.value : ""; // default empty if none selected
        ...
        if(domain_name == "" || domain_status == ""){
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo $site_url.$path_admin ?>/dashboard',
                data: {action: "create-domains", csrf_token: csrf_token_default, domain_name: domain_name, domain_status: domain_status},
                dataType: 'json',
                success: function (response) {
                    closeAllBootstrapModals();
    ...
    $('.modal-editItem-btn').click(function () {
        ...
        if(domain_id == "" || domain_name == "" || domain_status == ""){
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo $site_url.$path_admin ?>/dashboard',
                data: {action: "domains-edit", csrf_token: csrf_token_default, domain_id: domain_id, domain_name: domain_name, domain_status: domain_status},
                dataType: 'json',
                success: function (response) {
                    closeAllBootstrapModals();
