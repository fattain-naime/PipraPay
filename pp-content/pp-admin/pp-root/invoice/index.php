        ...
        <div class="row g-2 align-items-center">
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list align-items-center gap-3">
                    ...
                    <span onclick="load_content('Create Invoice','<?php echo $site_url.$path_admin ?>/invoice/create','nav-item-invoice')" class="<?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'invoice', 'create', $global_user_response['response'][0]['role']) ? '' : 'd-none' ?>">
                ...
                <div>
                    <div class="card p-2">
                        <ul class="nav nav-pills gap-2" role="tablist" id="statusTabs" style="font-weight: 500; font-size: .875rem;">
                            ...
                            <li class="nav-item">
                                <button class="nav-link" data-type="paid">
                                    Paid
                                </button>
                            </li>
                            <li class="nav-item">
                                <button class="nav-link" data-type="unpaid">
                                    Unpaid
                                </button>
                            </li>
                            <li class="nav-item">
...
<script data-cfasync="false">
        ...
        if(actionID == ""){
            ...
            if(my_action_confirmation_btn !== ""){
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?php echo $site_url.$path_admin ?>/dashboard',
                    data: {action: "invoice-bulk-action", csrf_token: csrf_token_default, actionID: actionID, selected_ids: JSON.stringify(selectedRows)},
                    dataType: 'json',
                    success: function (response) {
                        closeAllBootstrapModals();
    ...
    function load_data_list(page = 1){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo $site_url.$path_admin ?>/dashboard',
            data: {action: "invoice-list", csrf_token: csrf_token_default, search_input: search_input, show_limit: show_limit, tabType: tabType, page: page, filter_status: filter_status, filter_start: filter_start, filter_end: filter_end},
            dataType: 'json',
            success: function (res) {
                ...
                if (res.status === 'true') {
                    res.response.forEach(item => {
                        ...
                        if (allowEdit) {
                            redirectEdit = `style="cursor:pointer;" onclick="load_content('Edit Invoice','<?php echo $site_url.$path_admin ?>/invoice/edit?i_id=${item.id}','nav-item-invoice')"`;
                        ...
                        if (item.status === 'paid') badge = 'primary';
                        if (item.status === 'unpaid') badge = 'warning';
