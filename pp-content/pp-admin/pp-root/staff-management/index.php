...
<div class="page-header d-print-none" aria-label="Page header">
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            <div class="col">
            <!-- Page pre-title -->
                <div class="page-pretitle">Staff Management</div>
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list align-items-center gap-3">
                    ...
                    <span onclick="load_content('Create Staff','<?php echo $site_url.$path_admin ?>/staff-management/create','nav-item-staff-management')" class="<?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'create', $global_user_response['response'][0]['role']) ? '' : 'd-none' ?>">
                ...
                <div class="form-group mt-1">
                    ...
                    <div class="form-control-wrap">
                        <select class="form-select" id="model-bulkActionID">
                            ...
                            <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'edit', $global_user_response['response'][0]['role']) ? '<option value="activated">Activate Selected</option>' : '' ?>
                            <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'edit', $global_user_response['response'][0]['role']) ? '<option value="suspended">Suspend Selected</option>' : '' ?>
                        </select>
                    </div>
                </div>
            </div>
...
<script data-cfasync="false">
        ...
        if(actionID == ""){
            ...
            if(my_action_confirmation_btn !== ""){
                ...
                $.ajax({
                    ...
                    data: {action: "staff-bulk-action", csrf_token: csrf_token_default, actionID: actionID, selected_ids: JSON.stringify(selectedRows)},
                    dataType: 'json',
    ...
    function deleteItem(ItemID){
        ...
        if(my_action_confirmation_btn !== ""){
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo $site_url.$path_admin ?>/dashboard',
                data: {action: "staff-delete", csrf_token: csrf_token_default, ItemID: ItemID},
                dataType: 'json',
                success: function (response) {
                    closeAllBootstrapModals();
            ...
            show_action_confirmation_tab(btnClass, 'Delete Staff', 'Delete', 'btn-danger');
        }
    }
    ...
    function load_data_list(page = 1){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo $site_url.$path_admin ?>/dashboard',
            data: {action: "staff-management-list", csrf_token: csrf_token_default, search_input: search_input, show_limit: show_limit, page: page, filter_status: filter_status, filter_start: filter_start, filter_end: filter_end},
            dataType: 'json',
            success: function (res) {
                ...
                if (res.status === 'true') {
                    res.response.forEach(item => {
                        ...
                        if (allowEdit) {
                            redirectEdit = `style="cursor:pointer;" onclick="load_content('Edit Staff','<?php echo $site_url.$path_admin ?>/staff-management/edit?staff=${item.id}','nav-item-staff-management')"`;
                        }

                        if (allowPermission) {
                            redirectPermission = `style="cursor:pointer;" onclick="load_content('Permissions List','<?php echo $site_url.$path_admin ?>/staff-management/permissions-list?staff=${item.id}','nav-item-staff-management')"`;
                        }
                        ...
                        html += `
                            <tr data-id="${item.id}">
                                ...
                                <td ${redirectEdit}>${item.role.charAt(0).toUpperCase() + item.role.slice(1)}</td>
                                ...
                                <td class="text-end">
                                    <span class="dropdown" style="position: unset;">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end" style="">
                                            ...
                                            <a class="dropdown-item btnDeleteItem-${item.id} ${allowPermission ? '' : 'd-none'}" href="javascript:void(0)" ${redirectPermission}> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-prison"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M18 4v16" /><path d="M14 4v16" /><path d="M6 4v5" /><path d="M6 15v5" /><path d="M10 4v5" /><path d="M11 9h-6v6h6l0 -6" /><path d="M10 15v5" /><path d="M8 12h-.01" /></svg> Permissions </a>
                                        </div>
                                    </span>
                                </td>
                            </tr>
