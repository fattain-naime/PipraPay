    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            ...
            <!-- Page pre-title -->
                ...
                <h2 class="page-title">Permission List</h2>
            </div>
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list align-items-center gap-3">
                    ...
                    <span data-bs-target="#modal-createItem" data-bs-toggle="modal" class="<?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'assign_brand_to', $global_user_response['response'][0]['role']) ? '' : 'd-none' ?>">
                ...
                <div class="form-group mt-1">
                    ...
                    <div class="form-control-wrap">
                        <select class="form-select" id="model-bulkActionID">
                            ...
                            <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'edit_permission', $global_user_response['response'][0]['role']) ? '<option value="activated">Activate Selected</option>' : '' ?>
                            <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'edit_permission', $global_user_response['response'][0]['role']) ? '<option value="suspended">Suspend Selected</option>' : '' ?>
                        </select>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <form action="" class="staff-brand-add">
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary btn-5 ms-auto modal-createItem-btn">Confirm</button>
                </div>
            </form>
        </div>
    </div>
...
<script data-cfasync="false">
    $('.staff-brand-add').submit(function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {    console.error('AJAX Error');
                createToast({
        ...
        if(actionID == ""){
            ...
            if(my_action_confirmation_btn !== ""){
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?php echo $site_url.$path_admin ?>/dashboard',
                    data: {action: "staff-permission-bulk-action", csrf_token: csrf_token_default, actionID: actionID, selected_ids: JSON.stringify(selectedRows)},
                    dataType: 'json',
                    success: function (response) {
                        closeAllBootstrapModals();
    ...
    function deleteItem(ItemID){
        ...
        if(my_action_confirmation_btn !== ""){
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo $site_url.$path_admin ?>/dashboard',
                data: {action: "staff-permission-delete", csrf_token: csrf_token_default, ItemID: ItemID},
                dataType: 'json',
                success: function (response) {
                    closeAllBootstrapModals();
            ...
            show_action_confirmation_tab(btnClass, 'Delete Staff Permission', 'Delete', 'btn-danger');
        }
    }
    ...
    function load_data_list(page = 1){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo $site_url.$path_admin ?>/dashboard',
            data: {action: "staff-permissions", csrf_token: csrf_token_default, a_id: "<?php echo $staff_id?>", search_input: search_input, show_limit: show_limit, page: page, filter_status: filter_status, filter_start: filter_start, filter_end: filter_end},
            dataType: 'json',
            success: function (res) {
                ...
                if (res.status === 'true') {
                    res.response.forEach(item => {
                        ...
                        let allowEdit = <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'edit_permission', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;
                        let allowDelete = <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'staff', 'delete_permission_of', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;

                        let redirectEdit = '';
                        let redirectDelete = '';

                        if (allowEdit) {
                            redirectEdit = `style="cursor:pointer;" onclick="load_content('Edit Permission','<?php echo $site_url.$path_admin ?>/staff-management/edit-permissions?staff=${item.id}','nav-item-staff-management')"`;
                        }
                                ...
                                <td ${redirectEdit}>
                                    <div class="d-flex py-1 align-items-center">
                                        <div class="flex-fill">
                                            ...
                                            <div class="text-secondary">${item.brandname}</div>
                                        </div>
                                    </div>
                                </td>
