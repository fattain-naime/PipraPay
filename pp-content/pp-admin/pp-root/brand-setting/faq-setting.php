        ...
        <div class="row g-2 align-items-center">
            ...
            <!-- Page pre-title -->
                <div class="page-pretitle">
                    <ol class="breadcrumb breadcrumb-arrow mb-0">
                        ...
                        <li class="breadcrumb-item active"><a href="javascript:void(0)">FAQ Settings</a></li>
                    </ol>
                </div>
                <h2 class="page-title">FAQ Settings</h2>
            </div>
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list align-items-center gap-3">
                    ...
                    <span data-bs-target="#modal-createItem" data-bs-toggle="modal" class="<?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'faq_settings', 'create', $global_user_response['response'][0]['role']) ? '' : 'd-none' ?>">
                ...
                <div class="form-group mt-1">
                    ...
                    <div class="form-control-wrap">
                        <select class="form-select" id="model-bulkActionID">
                            ...
                            <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'faq_settings', 'delete', $global_user_response['response'][0]['role']) ? '<option value="deleted">Delete Selected</option>' : '' ?>
                            <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'faq_settings', 'edit', $global_user_response['response'][0]['role']) ? '<option value="activated">Active Selected</option>' : '' ?>
                            <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'faq_settings', 'edit', $global_user_response['response'][0]['role']) ? '<option value="inactivated">Inactive Selected</option>' : '' ?>
                        </select>
                    </div>
                </div>
            </div>
          ...
          <div class="modal-body">
            <div class="row mb-3 g-3">
              <div class="col-lg-12">
                ...
                <input type="text" class="form-control" name="faq-title" placeholder="FAQ Title">
              </div>

              <div class="col-lg-12">
                <label class="form-label">Description <span class="text-danger">*</span></label>
                <textarea class="form-control" name="faq-description" style=" min-height: 200px; " placeholder="FAQ Description"></textarea>
              </div>
            </div>
            ...
            <div class="form-selectgroup-boxes row g-3 mb-3">
              <div class="col-lg-6">
                <label class="form-selectgroup-item">
                  <input type="radio" name="faq-status" value="active" class="form-selectgroup-input" checked>
                  <span class="form-selectgroup-label d-flex align-items-center p-3">
                    ...
                    <span class="form-selectgroup-label-content">
                      ...
                      <span class="d-block text-secondary">If <strong>Active</strong>, this FAQ will be visible on the checkout page.</span>
                    </span>
                  </span>
                </label>
              </div>
              <div class="col-lg-6">
                <label class="form-selectgroup-item">
                  <input type="radio" name="faq-status" value="inactive" class="form-selectgroup-input">
                  <span class="form-selectgroup-label d-flex align-items-center p-3">
                    <span class="me-3">
                      <span class="form-selectgroup-check"></span>
                    </span>
                    <span class="form-selectgroup-label-content">
                      ...
                      <span class="d-block text-secondary">If <strong>Inactive</strong>, this FAQ will not be visible on the checkout page.</span>
                    </span>
                  </span>
                </label>
              </div>
...
<script data-cfasync="false">
        ...
        if(actionID == ""){
            ...
            if(my_action_confirmation_btn !== ""){
                ...
                $.ajax({
                    ...
                    data: {action: "faq-bulk-action", csrf_token: csrf_token_default, actionID: actionID, selected_ids: JSON.stringify(selectedRows)},
                    dataType: 'json',
    ...
    function deleteItem(ItemID){
        ...
        if(my_action_confirmation_btn !== ""){
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo $site_url.$path_admin ?>/dashboard',
                data: {action: "faq-delete", csrf_token: csrf_token_default, ItemID: ItemID},
                dataType: 'json',
                success: function (response) {
                    closeAllBootstrapModals();
            ...
            show_action_confirmation_tab(btnClass, 'Delete FAQ', 'Delete', 'btn-danger');
        }
    }
    ...
    function load_data_list(page = 1){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo $site_url.$path_admin ?>/dashboard',
            data: {action: "faq-list", csrf_token: csrf_token_default, search_input: search_input, show_limit: show_limit, page: page, filter_status: filter_status, filter_start: filter_start, filter_end: filter_end},
            dataType: 'json',
            success: function (res) {
                ...
                if (res.status === 'true') {
                    res.response.forEach(item => {
                        ...
                        let allowEdit = <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'faq_settings', 'edit', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;
                        let allowDelete = <?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'faq_settings', 'delete', $global_user_response['response'][0]['role']) ? 'true' : 'false' ?>;
                        let redirectEdit = '';
                        let redirectDelete = '';
    ...
    function openEditModel(itemID){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo $site_url.$path_admin ?>/dashboard',
            data: {action: "faq-info-byID", csrf_token: csrf_token_default, ItemID: itemID},
            dataType: 'json',
            success: function (response) {
                document.querySelector('.'+loaderSpinner).innerHTML = '';
                ...
                if (response.status === 'true') {
                    ...
                    modal.querySelector('input[name="faq-id"]').value = itemID || '';
                    modal.querySelector('input[name="faq-title"]').value = response.title || '';
                    modal.querySelector('textarea').value = response.description || '';

                    // Set status radio
                    const statusInputs = modal.querySelectorAll('input[name="faq-status"]');
                    statusInputs.forEach(input => {
                        input.checked = input.value === response.fstatus;
                    });
    ...
    $('.modal-createItem-btn').click(function () {
        ...
        // Set input values by name
        var faq_title = modal.querySelector('input[name="faq-title"]').value;
        var faq_description = modal.querySelector('textarea').value;

        var statusInput = modal.querySelector('input[name="faq-status"]:checked');
        var faq_status = statusInput ? statusInput.value : ""; // default empty if none selected
        ...
        if(faq_title == "" || faq_description == "" || faq_status == ""){
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo $site_url.$path_admin ?>/dashboard',
                data: {action: "faq-create", csrf_token: csrf_token_default, faq_title: faq_title, faq_description: faq_description, faq_status: faq_status},
                dataType: 'json',
                success: function (response) {
                    closeAllBootstrapModals();
                    ...
                    const radios = modal.querySelectorAll('input[name="faq-status"]');
                    if (radios.length > 0) {
    ...
    $('.modal-editItem-btn').click(function () {
        ...
        // Set input values by name
        var faq_id = modal.querySelector('input[name="faq-id"]').value;
        var faq_title = modal.querySelector('input[name="faq-title"]').value;
        var faq_description = modal.querySelector('textarea').value;
        ...
        if(faq_title == "" || faq_description == "" || faq_status == ""){
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo $site_url.$path_admin ?>/dashboard',
                data: {action: "faq-edit", csrf_token: csrf_token_default, faq_id: faq_id, faq_title: faq_title, faq_description: faq_description, faq_status: faq_status},
                dataType: 'json',
                success: function (response) {
                    closeAllBootstrapModals();
