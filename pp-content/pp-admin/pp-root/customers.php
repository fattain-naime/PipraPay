        ...
        <div class="row g-2 align-items-center">
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list align-items-center gap-3">
                    ...
                    <span data-bs-target="#modal-createItem" data-bs-toggle="modal" class="<?= hasPermission(json_decode($global_response_permission['response'][0]['permission'], true), 'customers', 'create', $global_user_response['response'][0]['role']) ? '' : 'd-none' ?>">
                ...
                <div>
                    <div class="card p-2">
                        <ul class="nav nav-pills gap-2" role="tablist" id="statusTabs" style="font-weight: 500; font-size: .875rem;">
                            ...
                            <li class="nav-item">
                                <button class="nav-link" data-type="manual">
                                    Manual
                                </button>
                            </li>
                            <li class="nav-item">
                                <button class="nav-link" data-type="checkout">
                                    Checkout
                                </button>
                            </li>
                        </ul>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="form-selectgroup-boxes row g-3 mb-3">
                ...
                <label class="form-selectgroup-item">
                  ...
                  <span class="form-selectgroup-label d-flex align-items-center p-3">
                    ...
                    <span class="form-selectgroup-label-content">
                      ...
                      <span class="d-block text-secondary">If <strong>Active</strong>, this customer can initiate payments.</span>
                    </span>
                  </span>
                </label>
              </div>
              <div class="col-lg-6">
                <label class="form-selectgroup-item">
                  <input type="radio" name="customer-status" value="suspend" class="form-selectgroup-input">
                  <span class="form-selectgroup-label d-flex align-items-center p-3">
                    <span class="me-3">
                      <span class="form-selectgroup-check"></span>
                    </span>
                    <span class="form-selectgroup-label-content">
                      <span class="form-selectgroup-title strong mb-1">Suspend</span>
                      <span class="d-block text-secondary">If <strong>Suspend</strong>, this customer can't initiate payments.</span>
                    </span>
                  </span>
                </label>
              </div>
            </div>
            <div class="row d-none" id="suspend-reason">
              <div class="col-lg-12">
                <div>
...
<script data-cfasync="false">
        ...
        if(actionID == ""){
            ...
            if(my_action_confirmation_btn !== ""){
                ...
                $.ajax({
                    ...
                    data: {action: "customers-bulk-action", csrf_token: csrf_token_default, actionID: actionID, selected_ids: JSON.stringify(selectedRows)},
                    dataType: 'json',
    ...
    function deleteItem(ItemID){
        ...
        if(my_action_confirmation_btn !== ""){
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo $site_url.$path_admin ?>/dashboard',
                data: {action: "customers-delete", csrf_token: csrf_token_default, ItemID: ItemID},
                dataType: 'json',
                success: function (response) {
                    closeAllBootstrapModals();
    ...
    function load_data_list(page = 1){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo $site_url.$path_admin ?>/dashboard',
            data: {action: "customer-list", csrf_token: csrf_token_default, search_input: search_input, show_limit: show_limit, tabType: tabType, page: page, filter_status: filter_status, filter_start: filter_start, filter_end: filter_end},
            dataType: 'json',
                    ...
                    res.response.forEach(item => {
                        ...
                        html += `
                            <tr data-id="${item.id}">
                                ...
                                <td ${redirectEdit}>${item.mobile}</td>
    ...
    function openEditModel(itemID){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo $site_url.$path_admin ?>/dashboard',
            data: {action: "customers-info-byID", csrf_token: csrf_token_default, ItemID: itemID},
            dataType: 'json',
            success: function (response) {
                document.querySelector('.'+loaderSpinner).innerHTML = '';
                ...
                if (response.status === 'true') {
                    ...
                    // Set status radio
                    const statusInputs = modal.querySelectorAll('input[name="customer-status"]');
                    statusInputs.forEach(input => {
                        input.checked = input.value === response.istatus;
                    });

                    // Show/hide suspend reason if needed
                    const suspendReasonDiv = modal.querySelector('#suspend-reason');
                    if (response.istatus === 'suspend') {
                        suspendReasonDiv.classList.remove('d-none');
                        suspendReasonDiv.querySelector('textarea').value = response.suspend_reason || '';
                    } else {
                        suspendReasonDiv.classList.add('d-none');
                    }
    ...
    $('.modal-createItem-btn').click(function () {
        ...
        var suspend_reason = modal.querySelector('textarea').value;
    ...
    $('.modal-editItem-btn').click(function () {
        ...
        if(customer_name == "" || customer_email == "" || customer_mobile == "" || customer_status == ""){
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo $site_url.$path_admin ?>/dashboard',
                data: {action: "customers-edit", csrf_token: csrf_token_default, customer_id: customer_id, name: customer_name, email: customer_email, mobile: customer_mobile, status: customer_status, suspend_reason: suspend_reason},
                dataType: 'json',
                success: function (response) {
                    closeAllBootstrapModals();
